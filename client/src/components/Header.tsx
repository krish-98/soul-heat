...
import { logout, updateToken, User } from '../app/features/authSlice'
import {
  calculateCartTotal,
  clearCart,
  getCart,
...
const Header = () => {
  ...
  const { totalItems, cartItems } = useAppSelector((store) => store.cart)
      ...
      try {
        if (user !== null) {
          const res = await fetch(
            `${import.meta.env.VITE_BACKEND_URL}/api/cart/all-items`,
            {
              method: 'GET',
              headers: {
  ...
  }, [user, cartItems?.length, totalItems])
